...
async def rainforest_fetch(session: aiohttp.ClientSession, asin: str) -> Tuple[Optional[float], Optional[str], Optional[str], str, Optional[int]]:
    ...
    async def fetch_once(domain_id: int):
        ...
        try:
            ...
            ri = js.get("request_info") if isinstance(js, dict) else None
            ...
            if not price:
                # Select lowest price from offers/prices
                candidates = []
                prices = p.get("offers") or p.get("prices") or []
                if isinstance(prices, list):
                    for item in prices:
                        if isinstance(item, dict):
                            pval = parse_price_obj(item.get("price"))
                            if pval:
